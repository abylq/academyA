        ...
        <form method="POST" action="{{ route('signup') }}">
            ...
            <div class="form-group">
                ...
                <input type="email" class="form-control @error('email') is-invalid @enderror" id="exampleInputEmail1" aria-describedby="emailHelp"
                       placeholder="Эл пошта" name="email">
                @error('email')
                <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
            ...
            <p><a href="{{ url('user/resetpassword') }}" class="forgotthepass">Құпиясөзді ұмыттым</a></p>
            <button type="submit" class="btn btn-primary">Тіркелу</button>
            <p class="ff2"><a href="{{url('user/login')}}">Порталға тіркелу</a></p>
